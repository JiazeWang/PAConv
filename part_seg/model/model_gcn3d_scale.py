...
class GCN_Fusion_surface(nn.Module):
    ...
    def forward(self, vertices):
        ...
        fm_g = F.relu(self.bn_g(self.conv_g(neighbor_index_g, vertices).transpose(2, 1)).transpose(2, 1), inplace= True)
...
class GCN_Fusion(nn.Module):
    ...
    def forward(self, vertices, input):
        ...
        fm_g = F.relu(self.bn_g(self.conv_g(neighbor_index_g, vertices, input).transpose(2, 1)).transpose(2, 1), inplace= True)
        output = torch.cat((fm_l, fm_g), 2)
        return output
