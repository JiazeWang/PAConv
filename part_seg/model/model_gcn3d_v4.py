...
class GCN3D(nn.Module):
    def __init__(self, class_num, support_num, neighbor_num):
        ...
        self.neighbor_num_l = 10
        self.neighbor_num_h = 100
        ...
        self.conv_0_h = gcn3d.Conv_surface(kernel_num= 128, support_num= support_num)
        self.conv_1_h = gcn3d.Conv_layer(128, 128, support_num= support_num)
        self.pool_1_h = gcn3d.Pool_layer(pooling_rate= 4, neighbor_num= 4)
        self.conv_2_h = gcn3d.Conv_layer(128, 256, support_num= support_num)
        self.conv_3_h = gcn3d.Conv_layer(256, 256, support_num= support_num)
        self.pool_2_h = gcn3d.Pool_layer(pooling_rate= 4, neighbor_num= 4)
        self.conv_4_h = gcn3d.Conv_layer(256, 512, support_num= support_num)

        self.bn0_h = nn.BatchNorm1d(128, momentum=0.1)
        self.bn1_h = nn.BatchNorm1d(128, momentum=0.1)
        self.bn2_h = nn.BatchNorm1d(256, momentum=0.1)
        self.bn3_h = nn.BatchNorm1d(256, momentum=0.1)

        dim_fuse = sum([128, 128, 256, 256, 512, 512, 16])
        self.conv1d_block = nn.Sequential(
            nn.Conv1d(dim_fuse*2, 512, 1),
    ...
    def forward(self,
                ...
                gt=None):
        l = self.forward_l(vertices, onehot, gt)
        h = self.forward_h(vertices, onehot, gt)
        f = torch.cat([l, h], dim= 2)

        conv1d_input = f.permute(0, 2, 1) #(bs, fuse_ch, vertice_num)
    ...
    def forward_l(self,
        ...
        neighbor_index = gcn3d.get_neighbor_index(vertices, self.neighbor_num_l)
        ...
        neighbor_index = gcn3d.get_neighbor_index(v_pool_1, self.neighbor_num_l)
        ...
        neighbor_index = gcn3d.get_neighbor_index(v_pool_2, self.neighbor_num_l)
    ...
    def forward_h(self,
        ...
        bs, vertice_num, _ = vertices.size()
        neighbor_index = gcn3d.get_neighbor_index(vertices, self.neighbor_num_h)

        fm_0 = F.relu(self.bn0_h(self.conv_0_h(neighbor_index, vertices).transpose(2, 1)).transpose(2, 1), inplace= True)
        fm_1 = F.relu(self.bn1_h(self.conv_1_h(neighbor_index, vertices, fm_0).transpose(2, 1)).transpose(2, 1), inplace= True)
        v_pool_1, fm_pool_1 = self.pool_1_h(vertices, fm_1)
        neighbor_index = gcn3d.get_neighbor_index(v_pool_1, self.neighbor_num_h)

        fm_2 = F.relu(self.bn2_h(self.conv_2_h(neighbor_index, v_pool_1, fm_pool_1).transpose(2, 1)).transpose(2, 1), inplace= True)
        fm_3 = F.relu(self.bn3_h(self.conv_3_h(neighbor_index, v_pool_1, fm_2).transpose(2, 1)).transpose(2, 1), inplace= True)
        #print("add bn")
        v_pool_2, fm_pool_2 = self.pool_2_h(v_pool_1, fm_3)
        neighbor_index = gcn3d.get_neighbor_index(v_pool_2, self.neighbor_num_h)

        fm_4 = self.conv_4_h(neighbor_index, v_pool_2, fm_pool_2)
        f_global = fm_4.max(1)[0] #(bs, f)
